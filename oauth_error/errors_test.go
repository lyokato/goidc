...
func TestErrorBasic(t *testing.T) {
	oe := NewOAuthError(ErrInvalidRequest, "this client_id is disabled")
	...
	expected_header := "Bearer realm=\"example.org\", error=\"invalid_request\", error_description=\"this client_id is disabled\""
	...
	expected_json := "{\"error\":\"invalid_request\",\"error_description\":\"this client_id is disabled\"}"
	if actual_json != expected_json {
		t.Errorf("Header:\n - got: %v\n - want: %v\n", actual_json, expected_json)
	}

	actual_query := oe.Query("e8a9bcdd")
	expected_query := "error=invalid_request&error_description=this+client_id+is+disabled&state=e8a9bcdd"
	if actual_query != expected_query {
		t.Errorf("Query:\n - got: %v\n - want: %v\n", actual_query, expected_query)
	}

	actual_query = oe.Query("")
	expected_query = "error=invalid_request&error_description=this+client_id+is+disabled"
	if actual_query != expected_query {
		t.Errorf("Query:\n - got: %v\n - want: %v\n", actual_query, expected_query)
	}
}
...
func TestErrorDetailed(t *testing.T) {
	oe := NewOAuthDetailedError(ErrInvalidRequest, "this client_id is disabled", "http://example.org/path/to/error/document")
	actual_error := oe.Error()
	expected_error := "invalid_request: this client_id is disabled"
	if actual_error != expected_error {
		t.Errorf("Error:\n - got: %v\n - want: %v\n", actual_error, expected_error)
